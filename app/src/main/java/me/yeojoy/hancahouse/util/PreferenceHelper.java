...
public class PreferenceHelper {
    ...
    public static boolean isCrawlerStatusOn(Context context) {
        return PreferenceUtil.getInstance(context).getInt(Constants.KEY_CRAWLER_STATUS, Constants.CRAWLER_STATUS_OFF)
                == Constants.CRAWLER_STATUS_ON;
    }

    public static void setCrawlerStatusOn(Context context) {
        PreferenceUtil.getInstance(context).putInt(Constants.KEY_CRAWLER_STATUS, Constants.CRAWLER_STATUS_ON);
    }

    public static void setCrawlerStatusOff(Context context) {
        PreferenceUtil.getInstance(context).putInt(Constants.KEY_CRAWLER_STATUS, Constants.CRAWLER_STATUS_OFF);
    }

    public static int getCrawlerDurationTime(Context context) {
        return PreferenceUtil.getInstance(context).getInt(Constants.KEY_TIMER_DURATION, Constants.TIMER_DEFAULT_HOUR);
    }

    public static void setCrawlerDurationTime(Context context, int hour) {
        PreferenceUtil.getInstance(context).putInt(Constants.KEY_TIMER_DURATION, hour);
    }
